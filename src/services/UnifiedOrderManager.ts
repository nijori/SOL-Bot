...
export class UnifiedOrderManager {
   ...
   * @returns 追加が成功したかどうか
   */
  public addExchange(exchangeId: string, exchangeService: ExchangeService, priority: number = 100): boolean {
    ...
    logger.info(`[UnifiedOrderManager] 取引所 ${exchangeId} (${exchangeService.getExchangeName()}) を追加しました`);
    return true;
  }
  ...
  public setExchangeActive(exchangeId: string, active: boolean): boolean {
    ...
    logger.info(`[UnifiedOrderManager] 取引所 ${exchangeId} を${active ? '有効' : '無効'}にしました`);
    return true;
  }
    ...
    switch (this.allocationConfig.strategy) {
      ...
      case AllocationStrategy.CUSTOM:
        ...
        if (!this.allocationConfig.customRatios || this.allocationConfig.customRatios.size === 0) {
          ...
          if (remainingAmount > 0.00001) { // 浮動小数点の誤差を考慮
            ...
            if (existingOrder) {
              ...
              allocatedOrders.set(highestPriorityExchange.id, { ...order, amount: remainingAmount });
            }
          }
        }
        break;
  ...
  public createOrder(order: Order): Map<string, string> {
    ...
    for (const [exchangeId, allocatedOrder] of allocatedOrders.entries()) {
      ...
      try {
        ...
        logger.info(`[UnifiedOrderManager] 取引所 ${exchangeId} で注文作成: ${orderId}, ${allocatedOrder.side} ${allocatedOrder.amount} ${allocatedOrder.symbol}`);
      } catch (error) {
        logger.error(`[UnifiedOrderManager] 取引所 ${exchangeId} での注文作成エラー: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  ...
  public cancelAllOrders(symbol?: string): number {
    ...
    for (const exchange of this.getActiveExchanges()) {
      const orders = exchange.oms.getOrders().filter(order => 
    ...
    logger.info(`[UnifiedOrderManager] ${symbol ? symbol + 'の' : ''}全注文キャンセル: ${cancelCount}件`);
    return cancelCount;
  }
  ...
  public async syncAllOrders(): Promise<void> {
    for (const exchange of this.getActiveExchanges()) {
      try {
        ...
        logger.error(`[UnifiedOrderManager] 取引所 ${exchange.id} の注文同期エラー: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
