...
export class GCPSecretManager implements SecretManagerInterface {
  ...
  constructor(config: GCPSecretManagerConfig = {}) {
    ...
    if (!this.projectId) {
      throw new Error('GCP Project IDが指定されていません。config.projectIdまたは環境変数GCP_PROJECT_IDを設定してください。');
    }
  ...
  async getSecret(key: string): Promise<string | null> {
    try {
      ...
      logger.error(`GCP Secret Manager取得エラー: ${error instanceof Error ? error.message : String(error)}`);
      return null;
    }
  }
  ...
  async setSecret(key: string, value: string): Promise<boolean> {
    try {
      ...
      logger.error(`GCP Secret Manager設定エラー: ${error instanceof Error ? error.message : String(error)}`);
      return false;
    }
  }
  ...
  async deleteSecret(key: string): Promise<boolean> {
    try {
      ...
      logger.error(`GCP Secret Manager削除エラー: ${error instanceof Error ? error.message : String(error)}`);
      return false;
    }
  }
  ...
  async hasSecret(key: string): Promise<boolean> {
    try {
      ...
      logger.error(`GCP Secret Manager確認エラー: ${error instanceof Error ? error.message : String(error)}`);
      return false;
    }
  }
} 