...
class MultiSymbolBacktestRunner {
   ...
   * 資金配分比率を計算
   */
  async calculateAllocationWeights() {
    ...
    switch (strategy) {
      ...
      case AllocationStrategy.CUSTOM:
        ...
        symbols.forEach((symbol) => {
          const weight = (this.config.symbolParams && this.config.symbolParams[symbol] && this.config.symbolParams[symbol].parameters && this.config.symbolParams[symbol].parameters.weight) || 1;
      ...
      case AllocationStrategy.VOLATILITY:
        try {
          ...
          // データロード用の関数
          const getInitialCandlesForSymbol = async (symbol) => {
            try {
              // 各シンボルのデータパスを構築
   ...
   * 各シンボル用のバックテストランナーを初期化
   */
  initializeSymbolRunners() {
    this.config.symbols.forEach((symbol) => {
      // シンボル固有の設定とグローバル設定をマージ
      const symbolSpecificConfig = this.config.symbolParams && this.config.symbolParams[symbol] || {};
...
並列数: ${this.parallelLimit}
    ...
    try {
      ...
      for (const { symbol, result } of results) {
        ...
        totalEquity += result.equity[result.equity.length - 1] && result.equity[result.equity.length - 1].equity || 0;
      ...
      // メモリモニタからピーク使用量を取得
      const memoryPeaks = this.memoryMonitor && this.memoryMonitor.getMemoryPeaks() || {
        rss: 0,
        heapTotal: 0,
        heapUsed: 0,
        external: 0
...
・ピーク使用量: ${(memoryPeaks.heapUsed / 1024 / 1024).toFixed(2)} MB
   ...
   * バックテスト結果をファイルに保存
   */
  async saveResultsToFile(symbolResults, combinedMetrics, executionStats) {
    try {
      // 結果を保存するディレクトリを作成
   ...
   */
  calculateCombinedMetrics(symbolResults) {
    // 各シンボルのメトリクスを集計
    const totalTrades = Object.values(symbolResults).reduce(
      (sum, result) => sum + result.trades.length,
      0
    ...
    const finalTotal = Object.values(symbolResults).reduce(
      (sum, result) => sum + (result.equity[result.equity.length - 1] && result.equity[result.equity.length - 1].equity || 0),
      0
    );
  ...
  static async runFromCli() {
    ...
    // デフォルト設定
    let symbols = ['SOL/USDT'];
    let timeframeHours = 4;
    let startDate = '2023-01-01';
    let endDate = '2023-12-31';
    let initialBalance = 10000;
    let allocationStrategy = AllocationStrategy.EQUAL;
    let slippage = 0.001;
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (arg === '--symbols' && i + 1 < args.length) {
        ...
        if (Object.values(AllocationStrategy).includes(allocArg)) {
          allocationStrategy = allocArg;
        }
