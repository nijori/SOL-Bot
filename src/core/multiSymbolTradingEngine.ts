...
export class MultiSymbolTradingEngine {
   ...
   */
  constructor(config: MultiSymbolEngineConfig, options: {
    isBacktest?: boolean;
    ...
    if (!this.quietMode) {
      logger.info(`[MultiSymbolTradingEngine] ${config.symbols.length}個のシンボルで初期化しました`);
      logger.info(`[MultiSymbolTradingEngine] 資金配分戦略: ${config.allocationStrategy || AllocationStrategy.EQUAL}`);
    }
  }
   ...
   * エクイティ履歴を取得
   */
  public getEquityHistory(): { timestamp: number; total: number; bySymbol: Record<string, number> }[] {
  ...
  public setSystemMode(mode: SystemMode): void {
    ...
    for (const engine of this.engines.values()) {
      if (engine['setSystemMode']) { // privateメソッドにアクセス
