...
import { generateAndSaveSampleData } from './generateSampleData';
import { BacktestRunner } from '../core/backtestRunner';
import { OptunaOptimizer } from '../optimizer/optunaOptimizer';
...
async function runSampleTest() {
  try {
    ...
    logger.error(`テスト実行中にエラーが発生しました: ${error instanceof Error ? error.message : String(error)}`);
    return false;
  }
}
