...
export class RealTimeDataProcessor extends EventEmitter {
  ...
  constructor(options: RealTimeDataProcessorOptions = {}) {
    ...
    this.dataTypes = new Set(options.dataTypes || [
    ...
    this.priorityDataTypes = new Set(options.priorityDataTypes || [
      RealTimeDataType.CANDLE,
      RealTimeDataType.TRADE
    ]);
    this.dynamicBufferSizeEnabled = options.dynamicBufferSizeEnabled !== undefined ? 
      options.dynamicBufferSizeEnabled : true;
    this.initialBufferFactor = options.initialBufferFactor || 1.0;
    
    // バッファとLRUキャッシュを初期化
    this.initializeBuffers();
    
    logger.info(`RealTimeDataProcessorを初期化しました（バッファサイズ: ${this.maxBufferSize}, 最大メモリ: ${this.maxMemoryMB}MB, LRUキャッシュ: ${this.enableLRUCache ? '有効' : '無効'}, 動的バッファサイズ: ${this.dynamicBufferSizeEnabled ? '有効' : '無効'}）`);
  }
  ...
  private initializeBuffers(): void {
    ...
    logger.debug(`バッファを初期化しました: ${this.buffers.size}バッファ, ${this.lruCaches.size}キャッシュ`);
  }
  ...
  public processData(data: RealTimeData): void {
    try {
      ...
      if ((this.symbols.size === 0 || this.symbols.has(data.symbol)) && 
  ...
  private checkMemoryUsage(): void {
    try {
      ...
      if (this.backPressureActive !== wasPreviouslyActive) {
        logger.info(`バックプレッシャー状態変更: ${wasPreviouslyActive ? '無効' : '有効'} -> ${this.backPressureActive ? '有効' : '無効'} (メモリ使用率: ${(memoryUsageRatio * 100).toFixed(1)}%)`);
      ...
      logger.debug(`メモリ使用状況: ${(usedHeapSize).toFixed(1)}MB / ${(totalHeapSize).toFixed(1)}MB (${(memoryUsageRatio * 100).toFixed(1)}%), バックプレッシャー: ${this.backPressureActive ? '有効' : '無効'}`);
      ...
      if (memoryUsageRatio > 0.95 && global.gc) {
        ...
        logger.warn(`高メモリ使用率のため明示的にGCを実行: ${(memoryUsageRatio * 100).toFixed(1)}%, GC実行時間: ${this.lastGCDuration}ms`);
      }
      
      // メモリ使用量が極めて高い場合は警告ログを出力
      if (memoryUsageRatio > 0.9) {
        logger.warn(`メモリ使用量が危険レベルに達しています: ${(usedHeapSize).toFixed(1)}MB / ${this.maxMemoryMB}MB (${(memoryUsageRatio * 100).toFixed(1)}%)`);
      }
    } catch (error) {
      logger.error(`メモリ使用量確認エラー: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private getMemoryUsageRatio(): number {
    try {
      ...
      logger.error(`メモリ使用率取得エラー: ${error instanceof Error ? error.message : String(error)}`);
      return 0;
    }
  }
    ...
    try {
      ...
      for (const [key, buffer] of this.buffers) {
        ...
        const baseSize = this.priorityDataTypes.has(dataType) ? 
          this.maxBufferSize * 1.5 : this.maxBufferSize;
        ...
        if (Math.abs(newSize - currentSize) > currentSize * 0.1) { // 10%以上変化した場合のみ更新
          ...
          logger.debug(`バッファサイズ調整: ${key} ${currentSize} -> ${newSize} (使用率: ${(bufferUsageRatio * 100).toFixed(1)}%, メモリ使用率: ${(memoryUsageRatio * 100).toFixed(1)}%)`);
        }
      }
    } catch (error) {
      logger.error(`バッファサイズ調整エラー: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private throttleEmit(bufferKey: string): void {
    ...
    const timer = setTimeout(() => {
      try {
        ...
        logger.error(`イベント通知エラー: ${error instanceof Error ? error.message : String(error)}`);
      }
  ...
  public stop(): void {
    ...
    logger.info(`リアルタイムデータ処理を停止しました。処理データ数: ${this.dataCount}, ピークメモリ使用量: ${finalMemoryInfo.peakMemoryUsage.toFixed(1)}MB`);
    ...
    try {
      ...
      if (data.type === RealTimeDataType.TICKER) {
        ...
        if (buffer && buffer.length > 0) {
          ...
          // データの妥当性チェック
          if (!lastData || !lastData.data || !lastData.data.price || !lastData.data.volume ||
          ...
          const volumeDiff = Math.abs((data.data.volume - lastData.data.volume) / lastData.data.volume);
      ...
      logger.error(`データフィルタリングエラー: ${error instanceof Error ? error.message : String(error)}`);
      return false;
    }
  }
  ...
  private updateCandleData(data: RealTimeData): void {
    try {
      ...
      // エラーをログに記録するが、例外は投げない
      logger.error(`ローソク足データ更新エラー: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   * @returns 最新データの配列
   */
  public getLatestData(symbol: string, dataType: RealTimeDataType, count: number = 1): RealTimeData[] {
  ...
  public getMemoryUsageInfo(): MemoryUsageInfo {
    try {
      ...
      logger.error(`メモリ使用情報取得エラー: ${error instanceof Error ? error.message : String(error)}`);
      return {
        totalHeapSize: 0,
        usedHeapSize: 0,
        memoryUsageRatio: 0,
