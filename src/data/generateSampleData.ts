...
function generateCandleData(options: {
  ...
  startTime.setUTCHours(Math.floor(startTime.getUTCHours() / timeframeHours) * timeframeHours, 0, 0, 0);
...
export async function generateAndSaveSampleData() {
  try {
    ...
    logger.info(`サンプルデータ生成: ${symbol} ${timeframeHours}h (${startDate.toISOString()} - ${endDate.toISOString()})`);
    ...
    for (let i = 0; i < candles.length; i += candlesPerBatch) {
      ...
      if (batchCandles.length > 0) {
        ...
        // Parquetに保存
        const saved = await dataStore.saveCandles(
          symbol,
          `${timeframeHours}h`,
          batchCandles
        );
        ...
        if (saved) {
          logger.info(`バッチ ${Math.floor(i / candlesPerBatch) + 1}: ${batchCandles.length}件のデータを保存しました`);
        } else {
    ...
    logger.error(`サンプルデータ生成エラー: ${error instanceof Error ? error.message : String(error)}`);
  }
}
