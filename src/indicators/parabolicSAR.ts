...
// YAML設定からパラメータを取得
const ACCELERATION_FACTOR_START = parameterService.get<number>('parabolicSAR.accelerationFactorStart', DEFAULT_ACCELERATION_FACTOR_START);
const ACCELERATION_FACTOR_INCREMENT = parameterService.get<number>('parabolicSAR.accelerationFactorIncrement', DEFAULT_ACCELERATION_FACTOR_INCREMENT);
const ACCELERATION_FACTOR_MAX = parameterService.get<number>('parabolicSAR.accelerationFactorMax', DEFAULT_ACCELERATION_FACTOR_MAX);
...
export class IncrementalParabolicSAR {
  ...
  update(candle: Candle): ParabolicSARResult {
    if (!this.isInitialized) {
      throw new Error('Parabolic SARが初期化されていません。初期化は最低2本のローソク足が必要です。');
    }
