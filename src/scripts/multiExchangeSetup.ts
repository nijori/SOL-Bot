...
function loadConfig(configPath: string) {
  try {
    ...
    logger.error(`設定ファイルの読み込みエラー: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  }
}
...
async function main() {
  ...
  for (const [exchangeId, exchangeConfig] of Object.entries(config.exchanges)) {
    try {
      ...
      logger.info(`取引所 ${exchangeId} (${exchangeService.getExchangeName()}) の初期化が完了しました`);
    } catch (error) {
      logger.error(`取引所 ${exchangeId} の初期化エラー: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  for (const symbol of symbols) {
    ...
    for (const [exchangeId, symbolInfoService] of symbolInfoServices.entries()) {
      try {
        // シンボルマッピングがある場合は使用
        const mappedSymbol = config.symbolMapping && config.symbolMapping[symbol] && config.symbolMapping[symbol][exchangeId]
          ? config.symbolMapping[symbol][exchangeId]
          : symbol;
        
        const symbolInfo = await symbolInfoService.getSymbolInfo(mappedSymbol);
        if (symbolInfo) {
          logger.info(`${exchangeId} の ${mappedSymbol} 情報: 最小注文量=${symbolInfo.minAmount}, 価格精度=${symbolInfo.pricePrecision}, 数量精度=${symbolInfo.amountPrecision}`);
        } else {
          logger.warn(`${exchangeId} の ${mappedSymbol} 情報を取得できませんでした`);
        }
      } catch (error) {
        logger.error(`${exchangeId} の ${symbol} 情報取得エラー: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    ...
    if (argv.mode === 'demo') {
      ...
      for (const [exchangeId, allocatedOrder] of allocatedOrders.entries()) {
        logger.info(`${exchangeId}: ${allocatedOrder.side} ${allocatedOrder.amount} ${allocatedOrder.symbol} @ ${allocatedOrder.price}`);
      }
    ...
    if (positions.size === 0) {
      ...
      for (const [exchangeId, exchangePositions] of positions.entries()) {
        for (const position of exchangePositions) {
          logger.info(`${exchangeId}: ${position.side} ${position.amount} @ ${position.entryPrice}, PnL: ${position.unrealizedPnl}`);
        }
      }
      ...
      if (totalPosition) {
        logger.info(`統合ポジション: ${totalPosition.side} ${totalPosition.amount} @ ${totalPosition.entryPrice}, PnL: ${totalPosition.unrealizedPnl}`);
      }
    }
  }
