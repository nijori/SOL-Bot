...
function printResults(results: Map<string, CalibrationResult>): void {
  ...
  // ヘッダー
  console.log('| シンボル | ATR% | ボラティリティ | 推奨ATR閾値 | トレイリング | グリッド幅 | ストップ距離 |');
  console.log('|----------|------|----------------|------------|------------|-----------|------------|');
...
function saveResultsToFile(results: Map<string, CalibrationResult>, outputDir: string): void {
  try {
    ...
    logger.error(`結果の保存に失敗しました: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function main() {
  try {
    ...
    logger.info(`${symbols.length}個のシンボルでATR%キャリブレーションを開始します (${timeframe})...`);
    ...
    for (const symbol of symbols) {
      ...
      try {
        ...
        if (!candles || candles.length < minLookbackCandles) {
          logger.warn(`${symbol}の十分なデータがありません: ${candles?.length || 0} < ${minLookbackCandles}`);
          continue;
        }
        ...
        logger.error(`${symbol}のデータ取得に失敗しました: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    ...
    logger.error(`キャリブレーション中にエラーが発生しました: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  }
}
