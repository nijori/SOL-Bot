...
import { RANGE_PARAMETERS, MARKET_PARAMETERS, RISK_PARAMETERS } from '../config/parameters';
import { parameterService } from '../config/parameterService';
import { calculateVWAP } from '../indicators/marketState';
...
const NET_POSITION_DELTA_MAX = parameterService.get<number>('rangeStrategy.netPositionDeltaMax', 0.15);
 ...
 */
function calculateDonchianChannel(candles: Candle[], period: number): { high: number, low: number } {
...
function createMakerOnlyLimitOrder(
  ...
  const adjustedPrice = side === OrderSide.BUY
...
function createIcebergOrders(
  ...
  for (let i = 0; i < chunks; i++) {
    ...
    orders.push(createMakerOnlyLimitOrder(
      symbol,
      side,
      price,
      amount,
...
function checkPositionImbalance(
  ...
  console.log(`[MeanRevertStrategy] ポジション偏り検出: ${(netPositionDelta * 100).toFixed(2)}%, ヘッジ実行: ${hedgeSide} ${hedgeAmount.toFixed(4)} @ ${currentPrice.toFixed(2)}`);
...
export function executeMeanRevertStrategy(
  ...
  try {
    ...
    console.log(`[MeanRevertStrategy] 市場分析: レンジ=${rangeLow.toFixed(2)}〜${rangeHigh.toFixed(2)}, ATR=${currentATR.toFixed(2)}, ATR%=${atrPercent.toFixed(2)}%, グリッド数=${gridLevelCount}`);
    ...
    if (currentPrice > rangeHigh * (1 + ESCAPE_THRESHOLD) && shortPositions.length > 0) {
      console.log(`[MeanRevertStrategy] レンジ上限エスケープ条件発動: ${currentPrice.toFixed(2)} > ${rangeHigh.toFixed(2)} * ${1 + ESCAPE_THRESHOLD}`);
      ...
      console.log(`[MeanRevertStrategy] レンジ下限エスケープ条件発動: ${currentPrice.toFixed(2)} < ${rangeLow.toFixed(2)} * ${1 - ESCAPE_THRESHOLD}`);
    ...
    if (positionRatio >= MAX_POSITION_SIZE) {
      console.log(`[MeanRevertStrategy] ポジション上限到達: ${(positionRatio * 100).toFixed(2)}% > ${(MAX_POSITION_SIZE * 100).toFixed(2)}%, 新規注文を生成しません`);
      return {
        strategy: StrategyType.RANGE_TRADING,
        signals,
        timestamp: Date.now()
    ...
    for (let i = 0; i < gridLevels.length; i++) {
      ...
      if (previousPrice < level && currentPrice >= level) {
        ...
        if (levelPosition > 0.5) {
          ...
          console.log(`[MeanRevertStrategy] 上昇クロス検出 レベル${i+1}/${gridLevelCount}: ${level.toFixed(2)}, 売り注文生成`);
      ...
      if (previousPrice > level && currentPrice <= level) {
        ...
        if (levelPosition < 0.5) {
          ...
          console.log(`[MeanRevertStrategy] 下降クロス検出 レベル${i+1}/${gridLevelCount}: ${level.toFixed(2)}, 買い注文生成`);
    ...
    if (currentPrice >= rangeHigh * 0.98 && currentPrice <= rangeHigh) {
      ...
      console.log(`[MeanRevertStrategy] レンジ上限付近: ${currentPrice.toFixed(2)} / ${rangeHigh.toFixed(2)}, 売り注文生成`);
      
      signals.push(createMakerOnlyLimitOrder(
        symbol,
        OrderSide.SELL,
        rangeHigh,
        sellAmount,
      ...
      console.log(`[MeanRevertStrategy] レンジ下限付近: ${currentPrice.toFixed(2)} / ${rangeLow.toFixed(2)}, 買い注文生成`);
    ...
    console.error(`[MeanRevertStrategy] エラー: ${error instanceof Error ? error.message : String(error)}`);
    return {
      strategy: StrategyType.RANGE_TRADING,
      signals: [],
      timestamp: Date.now(),
