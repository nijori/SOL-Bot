...
export function executeTrendStrategy(
  ...
  if (isBreakingUp && isStrongTrend && !hasLongPosition) {
    ...
    // リスクベースのポジションサイズを計算
    const positionSize = calculateRiskBasedPositionSize(accountBalance, currentPrice, stopPrice);
  ...
  if (hasLongPosition) {
    ...
    if (
      longAddOnCount < MAX_PYRAMIDS &&
      currentPrice > firstLongPosition.entryPrice + riskUnit * PYRAMID_THRESHOLD
    ) {
      // 新しいストップ価格
  ...
  if (hasShortPosition) {
    ...
    if (
      shortAddOnCount < MAX_PYRAMIDS &&
      currentPrice < firstShortPosition.entryPrice - riskUnit * PYRAMID_THRESHOLD
    ) {
      // 新しいストップ価格
      const addOnStopPrice = currentPrice + currentATR * TRAILING_STOP_FACTOR;
