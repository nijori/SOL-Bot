...
describe('Metrics Utility', () => {
  ...
  describe('新規追加メトリクス', () => {
    it('recordOrderLatency が注文レイテンシを正しくヒストグラムに記録する', () => {
      ...
      const metricData = collectMetrics();
      expect(metricData).toMatch(/solbot_order_latency_seconds_bucket{le="2",exchange="binance",order_type="market",symbol="SOLUSDT"} 1/);
      expect(metricData).toMatch(/solbot_order_latency_seconds_bucket{le="0.5",exchange="bybit",order_type="limit",symbol="BTCUSDT"} 1/);
      expect(metricData).toMatch(/solbot_order_latency_seconds_count{exchange="binance",order_type="market",symbol="SOLUSDT"} 1/);
      expect(metricData).toMatch(/solbot_order_latency_seconds_count{exchange="bybit",order_type="limit",symbol="BTCUSDT"} 1/);
    });
    ...
    it('recordExchangeError が取引所エラーを正しくカウントする', () => {
      ...
      const metricData = collectMetrics();
      expect(metricData).toMatch(/solbot_exchange_error_total{exchange="binance",code="429",endpoint="\/api\/v3\/order"} 2/);
      expect(metricData).toMatch(/solbot_exchange_error_total{exchange="bybit",code="10001",endpoint="\/private\/linear\/order\/create"} 1/);
    });
    ...
    it('recordEngineLoopDuration がエンジンループ処理時間を正しく記録する', () => {
      ...
      const metricData = collectMetrics();
      expect(metricData).toMatch(/solbot_engine_loop_duration_seconds{quantile="0.5",strategy="trend"}/);
