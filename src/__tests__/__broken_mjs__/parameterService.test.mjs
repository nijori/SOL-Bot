...
operation: simulation
  ...
  describe('processEnvVariables', () => {
    test('数値型への変換が正しく行われる', () => {
      ...
      // テスト対象のオブジェクト
      const testObj = {
        explicitNumber: '${TEST_NUMBER:number}',
        implicitNumber: '${TEST_NUMBER}',
        defaultNumber: '${TEST_NAN:number:789}',
        mixedCase: '${test_number:number}'
      };
    ...
    test('真偽値への変換が正しく行われる', () => {
      ...
      // テスト対象のオブジェクト
      const testObj = {
        explicitTrue: '${TEST_TRUE:boolean}',
        implicitTrue: '${TEST_TRUE}',
        yesValue: '${TEST_YES:boolean}',
        onValue: '${TEST_ON:boolean}',
        explicitFalse: '${TEST_FALSE:boolean}',
        implicitFalse: '${TEST_FALSE}',
        noValue: '${TEST_NO:boolean}',
        offValue: '${TEST_OFF:boolean}',
    ...
    test('文字列型への変換が正しく行われる', () => {
      ...
      // テスト対象のオブジェクト
      const testObj = {
        explicitString: '${TEST_STRING:string}',
        forceNumberAsString: '${TEST_NUMBER_STRING:string}',
        forceBooleanAsString: '${TEST_BOOLEAN_STRING:string}'
      };
    ...
    test('複雑なオブジェクトと配列が正しく処理される', () => {
      ...
      const testObj = {
        simpleValues: {
          num: '${TEST_NUMBER:number}',
          bool: '${TEST_BOOLEAN:boolean}',
          str: '${TEST_STRING:string}'
        },
        arrayValues: [
          '${TEST_NUMBER:number}',
          '${TEST_BOOLEAN:boolean}',
          {
            nestedValue: '${TEST_STRING:string}'
          }
        ]
      };
    ...
    test('環境変数が存在しない場合、デフォルト値が使用される', () => {
      ...
      const testObj = {
        ...
        oldFormat: '${NON_EXISTENT:string:legacy default}'
      };
  ...
  describe('DI対応後のテスト', () => {
    ...
    it('初期パラメータを使用して初期化できる', () => {
      ...
      // 初期パラメータが反映されていることを確認
      expect(instance.get('custom.param1')).toBe(100);
      expect(instance.get('custom.param2')).toBe('value');
    });
    ...
    it('createMockParameterService関数でモックインスタンスを作成できる', () => {
      ...
      expect(mockService.get('test.value1')).toBe(999);
      expect(mockService.get('test.value2')).toBe('mock');
    });
    ...
    it('updateParameters関数で設定を更新できる', () => {
      ...
      // 初期値を確認
      expect(instance.get('market.donchian_period')).toBe(30);
      ...
      // 更新後の値を確認
      expect(instance.get('market.donchian_period')).toBe(50);
    });
  ...
  describe('並列バックテスト時のレース条件対策', () => {
    it('複数のバックテストプロセスでそれぞれの設定が分離される', () => {
      ...
      // 設定が分離されていることを確認
      expect(bt1Service.get('market.donchian_period')).toBe(15);
      expect(bt2Service.get('market.donchian_period')).toBe(20);
      expect(parameterService.get('market.donchian_period')).toBe(14); // グローバルインスタンスは変更されない
    });
  });
  ...
  describe('テスト用モック注入', () => {
    it('戦略へのDI注入が機能する', () => {
      ...
      // それぞれ異なる値を返すことを確認
      expect(realStrategy.getTrailingStopFactor()).toBe(1.0); // シングルトンの元の値
      expect(mockedStrategy.getTrailingStopFactor()).toBe(2.0); // モックの値
    });
  });
});
