...
OrderManagementSystem.prototype.stopMonitoring = jest.fn().mockImplementation(function() {
  ...
  describe('注文配分テスト', () => {
    ...
    test('均等分割配分方式', () => {
      ...
      // 分割された注文を検証
      const calls = OrderManagementSystem.prototype.createOrder;
    ...
    test('カスタム配分方式', () => {
      ...
      // 分割された注文を検証
      const calls = OrderManagementSystem.prototype.createOrder;
      expect(calls[0][0].amount).toBe(7); // binance * 0.7 = 7
      expect(calls[1][0].amount).toBe(3); // bybit * 0.3 = 3
    });
  });
