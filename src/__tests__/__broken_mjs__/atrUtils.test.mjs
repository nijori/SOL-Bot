...
import { calculateATR, getFallbackATR, isATRTooSmall, getValidStopDistance } from '../../utils/atrUtils.js';
...
jest.mock('../../config/parameterService.js', () => {
  return {
    parameterService: {
      getParam: jest.fn((key, defaultValue) => {
        const params = {
...
// テスト用のダミーローソク足データ生成
function generateDummyCandles(count, basePrice) {
  const candles = [];
...
function generateZeroVolatilityCandles(count, basePrice) {
  const candles = [];
