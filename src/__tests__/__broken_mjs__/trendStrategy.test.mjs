...
describe('executeTrendStrategy', () => {
  // テスト用のモックデータを作成する関数
  function createMockCandles(length, startPrice, trend) {
    const candles = [];
    ...
    for (let i = 0; i < length; i++) {
      // トレンドに応じて価格変動を設定
      const change = trend === 'up' ? (Math.random() > 0.5 ? 5 : 3) : (Math.random() > 0.5 ? -5 : -3);
      price += change;
