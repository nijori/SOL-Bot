...
class MockExchangeService implements Partial<ExchangeService> {
  ...
  mapOrderTypeToCCXT(): string { return 'limit'; }
  mapCCXTToOrderType(): OrderType { return OrderType.LIMIT; }
  async fetchOrder(): Promise<any> { return null; }
  async fetchOrderAndConvert(): Promise<Order | null> { return null; }
  supportsFeature(): boolean { return true; }
  supportsOCO(): boolean { return true; }
  async getMarketInfo(): Promise<any> { return null; }
  async fetchTicker(): Promise<any> { return null; }
}
...
jest.mock('../../services/UnifiedOrderManager', () => {
  ...
  class MockUnifiedOrderManager {
    private exchanges = new Map<string, { id: string; exchangeService: any; active: boolean; priority: number }>();
    ...
    addExchange(exchangeId: string, exchangeService: ExchangeService, priority: number = 100): boolean {
