...
describe('MultiSymbolTradingEngine', () => {
  let mockEngines: Map<string, jest.Mocked<TradingEngine>>;
  ...
  beforeEach(() => {
    ...
    // モック実装を作成
    (TradingEngine as jest.Mock).mockImplementation((options) => {
      ...
      mockEngines.set(symbol, mockEngine as unknown as jest.Mocked<TradingEngine>);
      return mockEngine;
    });
    
    // UnifiedOrderManagerのモック実装
    (UnifiedOrderManager as jest.Mock).mockImplementation(() => ({
      addExchange: jest.fn(),
  ...
  test('ポートフォリオリスク分析が正しく行われる', async () => {
    ...
    btcEngine!.getPositions.mockReturnValue([
      { symbol: 'BTC/USDT', side: OrderSide.SELL, amount: 0.5, currentPrice: 30000, entryPrice: 31000 }
    ]);
  ...
  test('シンボル間の相関行列が計算される', async () => {
    ...
    (engine as any).updateCorrelationMatrix();
