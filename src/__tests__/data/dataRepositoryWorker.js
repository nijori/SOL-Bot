...
var dataRepository_js_js_1 = require("../../data/dataRepository.js.js");
var types_js_js_1 = require("../../core/types.js.js");
// コマンドライン引数の取得
...
function createMockOrders(count) {
    ...
    for (var i = 0; i < count; i++) {
        ...
        orders.push({
            ...
            symbol: testSymbol,
            type: types_js_js_1.OrderType.LIMIT,
            side: Math.random() > 0.5 ? types_js_js_1.OrderSide.BUY : types_js_js_1.OrderSide.SELL,
            price: price,
            amount: Math.random() * 1,
            timestamp: timestamp,
            status: types_js_js_1.OrderStatus.OPEN
        });
    }
    return orders;
}
...
}(dataRepository_js_js_1.DataRepository));
// メインの実行関数
function run() {
