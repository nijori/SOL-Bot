...
describe('TrendFollowStrategy', () => {
  ...
  // テスト用のキャンドルデータを生成
  const generateTestCandles = (count: number, startPrice: number = 100, trend: 'up' | 'down' | 'flat' = 'flat'): Candle[] => {
  ...
  describe('executeTrendFollowStrategy', () => {
    ...
    it('データ不足時は空のシグナルを返す', () => {
      ...
      const result = trendFollowStrategyModule.executeTrendFollowStrategy(candles, 'SOLUSDT', [], 10000);
    ...
    it('stopPrice==undefinedの場合に適切にスキップ', () => {
      ...
      const result = trendFollowStrategyModule.executeTrendFollowStrategy(candles, 'SOLUSDT', positions, 10000);
      
      // stopPriceがundefinedなので警告が出るはず
      expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('ストップ価格が設定されていません'));
