...
// モックデータを作成するヘルパー関数
function createMockCandles(count: number, basePrice: number, trend: 'up' | 'down' | 'range' = 'range'): Candle[] {
...
function createDefiniteTrendCandles(count: number, basePrice: number, trend: 'up' | 'down'): Candle[] {
  const candles: Candle[] = [];
  let currentPrice = basePrice;
...
describe('MarketState Indicators', () => {
  ...
  describe('インクリメンタルEMA/ATR計算のテスト', () => {
    test('インクリメンタル計算と通常計算で同じ結果が得られる', () => {
      ...
      if (result1.indicators.shortTermEma && emaShortResult.length > 0) {
        expect(Math.abs(result1.indicators.shortTermEma - emaShortResult[emaShortResult.length - 1]) / emaShortResult[emaShortResult.length - 1]).toBeLessThan(0.05); // 5%以内の誤差を許容
      }
      
      if (result1.indicators.longTermEma && emaLongResult.length > 0) {
        expect(Math.abs(result1.indicators.longTermEma - emaLongResult[emaLongResult.length - 1]) / emaLongResult[emaLongResult.length - 1]).toBeLessThan(0.05); // 5%以内の誤差を許容
      }
      
      if (result1.indicators.atr && atrResult.length > 0) {
        expect(Math.abs(result1.indicators.atr - atrResult[atrResult.length - 1]) / atrResult[atrResult.length - 1]).toBeLessThan(0.10); // 10%以内の誤差を許容
      }
    });
    ...
    test('新しいローソク足が追加されても適切に計算が継続される', () => {
      ...
      // 追加のローソク足（数を減らす）
      const additionalCandles = createMockCandles(5, initialCandles[initialCandles.length - 1].close, 'up');
      ...
      if (atr3 > 0) {
        expect(Math.abs(atr2 - atr3) / atr3).toBeLessThan(0.10);
      }
    });
  ...
  describe('市場環境分析のテスト', () => {
    test('上昇トレンドを正しく識別する', () => {
      ...
      console.log(`短期EMA: ${result.indicators.shortTermEma}, 長期EMA: ${result.indicators.longTermEma}`);
      ...
      if (!isUpTrend) {
        ...
        console.log(`最終価格: ${candles[candles.length - 1].close}, 初期価格: ${candles[0].close}`);
        console.log(`価格差: ${(candles[candles.length - 1].close - candles[0].close) / candles[0].close * 100}%`);
