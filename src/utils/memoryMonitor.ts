...
export class MemoryMonitor {
  ...
  public startMonitoring(intervalMs: number = 1000): void {
    ...
    this.intervalId = setInterval(() => {
      ...
      if (snapshot) {
        logger.debug(`[MemoryMonitor:${this.label}] Heap: ${snapshot.heapUsed}MB / ${snapshot.heapTotal}MB, RSS: ${snapshot.rss}MB`);
      }
    }, intervalMs);
  }
...
回収量: ${((memUsage.heapUsed - afterGcMemUsage.heapUsed) / 1024 / 1024).toFixed(2)}MB
      ...
      logger.warn('ガベージコレクションを強制実行できません。--expose-gc オプションでNode.jsを起動してください。');
    }
