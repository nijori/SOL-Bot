...
│   ├── crontab-data-lifecycle.txt # データライフサイクルcronジョブ設定サンプル
│   └── test-sharding.js        # Jest並列テスト実行スクリプト
├── src/                        # ソースコード
│   ├── __tests__/              # テストコード
...
- **自動アーカイブ**: 90日以上経過したデータをS3へ自動移行
- **段階的保存**: ローカル → S3 → Glacier の段階的移行
- **コスト最適化**: 古いデータは低コストストレージへ移行
- **cronジョブ対応**: 定期的な自動実行（推奨: 毎日深夜2時）
- **IAMロール統合**: EC2インスタンスロールによる安全なアクセス
...
### DAT-015: データライフサイクル管理実装 (2025-06-17完了)

- **自動アーカイブ機能**: 90日以上経過したデータファイルをS3に自動移行
- **IAMロール統合**: EC2インスタンスロール(solbot-shared-role)によるS3アクセス
- **S3バケット構成**: `solbot-data`（通常アーカイブ）、`solbot-archive-nijori`（長期保存）
- **実機テスト完了**: EC2環境で2ファイルのS3アップロード成功確認
- **cronジョブ対応**: 定期実行用のcron設定サンプル提供（scripts/crontab-data-lifecycle.txt）