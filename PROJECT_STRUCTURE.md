...
│   │   ├── atrUtils.ts         # ATR計算とフォールバック機能の共通ユーティリティ
│   │   └── todoValidator.ts    # Todoタスク形式検証ユーティリティ
│   ├── data/                   # データ処理、永続化
...
│   │   ├── fetchMultiTimeframeData.ts # 複数時間足データ取得スクリプト
│   │   └── todo-lint.ts        # Todoタスク形式検証スクリプト
│   ├── services/               # 外部サービス連携
...
- **atrUtils.ts**: ATR計算とフォールバック共通ユーティリティ
  - 複数戦略で使用されるATR計算ロジックを集約
  - ATR=0の場合のフォールバック値計算機能
  - 期間調整可能なATR計算機能
- **todoValidator.ts**: Todoタスク形式検証ユーティリティ
  - .todo/*.mdcファイルのパースと検証機能
  - 複数の検証ルール実装（タスクID重複、進捗/Health一貫性、期限切れ、依存参照など）
  - Jest単体テスト完備
  - CI/CDパイプラインとの連携を想定した設計（エラー時に終了コード1を返す）
...
- **Gitleaksによる機密情報スキャン**: `.pre-commit-config.yaml`と`.gitleaks.toml`を使用して、コミット前に自動的に機密情報（APIキー、認証情報など）をスキャンする。詳細は`docs/gitleaks-setup.md`を参照。
  - シークレットスキャン設定: 暗号通貨取引所（Binance、Coinbase、Kraken）のAPIキー検出ルール
  - ソラナウォレットキーの検出ルール
  - AWS認証情報の検出ルール
  - テストデータやサンプルコードの除外設定
- **シークレットマネージャー**: API Key、Secret Keyなどの機密情報を安全に管理するための抽象シークレットマネージャーを実装
  - 複数バックエンド対応: AWS Parameter Store、GCP Secret Manager、環境変数、ファイルベース
  - 開発/テスト環境と本番環境での異なる実装の切り替え
  - 設定による自動選択機能
- **非rootユーザー実行**: 本番環境のDockerコンテナでは特権のない専用ユーザー（solbot）でアプリケーションを実行
- **最小権限の原則**: 各コンポーネントが必要最小限の権限のみを持つよう設計
...
ソースコード内のscriptsディレクトリには以下のコマンドラインツールが含まれています:

- **fetchMultiTimeframeData.ts**: 複数の時間足でのデータ取得コマンドラインツール
  - 1分足、15分足、1時間足、日足の各時間足データを一括取得
  - 期間指定やシンボル指定機能
  - 実行方法: `npm run fetch-data:all` または `npm run fetch-data:1m`
- **todo-lint.ts**: Todoタスク形式検証コマンドラインツール
  - .todo/*.mdcファイルの形式検証
  - 様々な検証チェック（タスクID重複、進捗/Health不整合、期限切れなど）
  - エラータイプ別にカラー表示で問題報告
  - 実行方法: `npm run todo-lint`
  - 実装済みオプション:
    - `--quiet`: 成功メッセージを表示せず、エラーのみ出力
    - `--format=json`: 検証結果をJSON形式で出力
    - `--fix`: 自動修正可能な問題を修正（フレームワークのみ実装済み、修正ロジックは将来実装予定）
  - 進捗率形式の検証: 標準形式(0%, 25%, 50%, 75%, 100%)に加え、10%や33%などの柔軟な形式にも対応
- **fix-todo-issues.ts**: Todoタスクの問題を自動修正するスクリプト
  - 完了済みタスクの進捗率フィールドがない場合、100%を自動追加
  - 完了済みタスクのHealth状態が✅でない場合、修正
  - タスクIDの重複を検出し、番号を変更して修正（古いものにサフィックスを追加）
  - 存在しないタスクIDへの依存参照を自動削除
  - バックアップ作成機能と自動復旧メカニズム
  - 実行方法: `npm run todo-fix`
  - エラー時には処理中断し、バックアップから復元

プロジェクトルートのscriptsディレクトリには、環境構築や運用のための以下のシェルスクリプトが含まれています:
...
GitHub Actionsは、リポジトリ内で直接ワークフローを自動化するための強力なCI/CDプラットフォームです。現在のプロジェクトでは主に以下の3つのワークフローが実装されています：

#### 1. todo-check.yml（Todoフォーマット検証）

**実装内容**:
- Todoファイル（.todo/*.mdc）が変更されたときに自動実行
- タスク形式が正しいかをチェック（todo-lintツールを使用）
- タスクIDの一意性、必須フィールドの存在、形式の一貫性を検証

**メリット**:
- フォーマットの不整合を早期に検出し、混乱を防止
- タスク管理の標準化を強制
- 人為的なミスによる不正確なタスク追跡を防止
- チームメンバー全員が同じ形式でタスクを管理できる

#### 2. pr-todo-auto-update.yml（PR連携自動更新）

**実装内容**:
- PRがマージされたときに自動実行
- PRタイトルまたは本文から関連するタスクIDを抽出
- 対応するTodoタスクを自動的に完了状態にマーク
- ステータス更新（チェックボックス[x]、進捗率100%、健全性✅）

**メリット**:
- タスク状態とコード変更の一貫性を自動的に維持
- 手動更新の手間を省き、タスク管理の負担を軽減
- マージ時に自動的に記録が更新されるため、状態の追跡が容易
- 「コードがマージされたのにタスクが未完了のまま」という状態を防止

#### 3. .github/PULL_REQUEST_TEMPLATE.md（PRテンプレート）

**実装内容**:
- PR作成時に自動的にテンプレートが適用される
- タスクID参照を必須項目として明示
- コードレビューチェックリストを提供
- タスクとPRの関連付けを促進

**メリット**:
- タスクとコード変更の明示的な紐付けを促進
- コードレビューの品質向上
- 一貫性のあるPR説明フォーマットの維持
- トレーサビリティの向上（どのコード変更がどのタスクに対応するか明確）

#### 総合的なメリット

1. **自動化による効率化**:
   - 定型的なタスク管理作業を自動化し、開発者の負担を軽減
   - 手動更新によるミスや漏れを防止

2. **一貫性の維持**:
   - すべてのタスクが同じルールで管理され、データの信頼性が向上
   - チーム全体での統一されたプラクティスの適用

3. **トレーサビリティの向上**:
   - コード変更とタスクの関連付けが明確に記録
   - プロジェクト進捗の可視化が容易
...
これらのGitHub Actionsワークフローにより、タスク管理の精度と効率が大幅に向上し、開発プロセス全体がスムーズになります。また、新メンバーの参加時も、標準化されたプロセスに従うことで迅速に適応できます。

### Todo検証ツール

コマンドラインからTodoファイルの形式を検証するtodo-lintツールを実装しています:
...
エラーはタイプ別にグループ化され、カラー表示されます。CI/CDパイプラインとの連携を想定し、エラーがある場合は終了コード1を返します。
...
現行のSOL-Botはフレームワークとして他の暗号資産ペアへの拡張が可能です。TradingEngine、BacktestRunner、OrderManagementSystem、ExchangeServiceの各コンポーネントは既にシンボルを引数で受け取り、ロジック内のハードコードは最小限に抑えられています。

### マルチアセット化に向けた主要課題

実際に複数通貨ペアでの運用を開始するには、以下の課題を解決する必要があります：

1. **取引単位とロットサイズの抽象化**
   - ccxtのmarketオブジェクトからprecision.amount、limits.amount.minなどを取得
   - OrderSizingServiceの新設（現状の固定値amountを通貨ごとに適切に調整）

2. **取引パラメータのペア別設定**
   - スプレッド、手数料、スリッページなどをシンボルごとに設定
   - パラメータYAMLのネスト構造化（例：backtest.slippage.SOLUSDT: 0.001）

3. **ボラティリティに応じたパラメータ調整**
   - ATR%ベースのパラメータ（GRID_ATR_MULTIPLIER等）を通貨ペアごとに自動調整
   - 直近N日間のATR%平均から動的に計算する関数の実装

4. **データストアのマルチシンボル対応**
   - ディレクトリ構造の動的生成（data/candles/<symbol>/<timeframe>.parquet）

5. **ポートフォリオリスク管理**
   - シンボル横断でのリスク集中管理
   - 最大同方向デルタ、日次総VaR制限などの実装

### マルチアセット実装フェーズ

#### フェーズ1: 基本対応（最小MVP、2週間）

1. **OrderSizingService実装** (UTIL-002)
   - ccxt.market情報を活用したロットサイズと精度計算
   - 通貨ペア別の最小取引単位を自動取得
   - BTCなど高額通貨でも適切なリスク量計算

2. **設定ファイル構造変更** (CONF-006)
   - parameters.yamlをネスト構造に変更
   - 通貨ペア別設定セクション追加
   - デフォルト値とペア固有値の階層設計

3. **引数インターフェース拡張** (CLI-001)
   - npm run backtest -- --symbol BTCUSDT形式の引数対応
   - 複数シンボル同時バックテスト（カンマ区切り）
   - CLI引数のREADME文書化

#### フェーズ2: 動的パラメータ調整（3週間）

1. **ATR%自動キャリブレーション** (ALG-040)
   - 通貨ペア別ボラティリティ特性の分析機能
...
1. **PortfolioRiskGuard実装** (RISK-006)
   - シンボル横断でのポジション集中度監視
...
- **v1.0.0**: マルチシンボル並列運用（同一口座内、各エンジン内でのリスク管理）
- **v1.1.x**: PortfolioRiskGuard、共通データストア、ペア別パラメータ自動推定
- **v1.2.x**: クロスマージン口座対応、為替建て口座のPnL換算

## Todoタスク管理システムの評価と改善提案

現行のTodoタスク管理システムは`.todo/`ディレクトリを中心に運用されており、GitHub Actionsと連携して自動化されています。このシステムの評価と改善提案を以下にまとめます。

### 長所

1. **明確な構造化**:
   - backlog.mdc/sprint.mdc/archive.mdcの分離により、タスクのライフサイクルが明確
   - フォーマットが統一されており、情報が一貫性を持つ

2. **GitHub Actions連携**:
   - PRとタスクの紐付けが自動化されている
   - タスク形式の検証が自動化されている
   - マージ時の自動更新機能によりタスク状態が常に最新

3. **豊富なメタデータ**:
   - 期限、担当者、依存関係、ラベル、健全性など多角的なタスク情報
   - 進捗率の視覚化

### 短所

1. **冗長性と二重管理リスク**:
   - 同一タスクがbacklog.mdcとsprint.mdcに重複して存在する可能性
   - 重複タスクの状態が不一致になるリスク

2. **状態の不整合**:
   - 進捗率とヘルスステータスの整合性チェックが不足
   - 期限切れタスクの自動検出・通知機能がない

3. **検索性の制限**:
   - 特定のラベルやオーナーでのフィルタリングが容易でない
   - 進捗状況の横断的な把握が困難

### 改善提案

1. **Todo-lintツールの開発** (TST-004):
   - タスクIDの重複チェック
   - 進捗率とヘルスステータスの整合性検証
   - 期限切れタスクの検出と通知

2. **Todoデータベース化** (INF-024、新規):
   - YAMLファイルをJSONデータベースに変換
   - GraphQL APIによる柔軟なクエリ機能
   - デスクトップ/Webアプリでの可視化

3. **メトリクス分析強化** (METR-003、新規):
   - タスク完了率や平均所要時間の分析
   - 依存関係グラフの可視化
   - ベロシティ計測と予測

4. **自動移行の改善** (CICD-004、新規):
   - スプリント開始時のタスク自動移行機能
   - 完了タスクのアーカイブ自動化
   - 期限切れタスクのエスカレーション

これらの改善により、タスク管理の効率化、ヒューマンエラーの削減、プロジェクト進捗の可視化が向上します。特に初期段階ではTodo-lintの実装を優先し、タスクの整合性を確保することが重要です。
