...
function fixJestMocksInFile(filePath) {
  ...
  try {
    ...
    content = content.replace(mockImplPattern, (match, implementation) => {
      ...
      let fixedImplementation = implementation
        .replace(
          /execute\)\.mockResolvedValue\(\{\s*signals\)\s*\}\)/,
    ...
    // 5. その他の構文エラー修正
    content = content
      .replace(/this\s*=\s*this\.config\.symbol/, 'this.config.symbol')
      .replace(/\}\s*\);(\s*\n+\s*\}\s*\);)/, '});\n$1');
...
function fixSetupJestFile() {
  ...
  if (fs.existsSync(setupFilePath)) {
    ...
    try {
      ...
      content = content.replace(
        /jest\.mock\(['"]([^'"]+)['"]\s*,\s*\(\)\s*=>\s*\{([\s\S]*?)\}\s*,\s*\{\s*virtual\s*:\s*true\s*\}\s*;/g,
        (match, modulePath, body) => {
          ...
          let fixedBody = body.replace(
            /(\s*return\s*\{[\s\S]*?)__esModule\s*:\s*true\s*,/g,
            '$1__esModule: true,'
          );
      ...
      content = content.replace(
        /execute\)\.mockResolvedValue\(\{\s*signals\s*:\s*\[\]\s*\}\)/g,
        'execute: jest.fn().mockResolvedValue({ signals: [] })'
      );
