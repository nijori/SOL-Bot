...
### コントラクトテストとユニットテストの使い分け

CommonJS変換プロセスで明らかになった課題に対応するため、以下の2種類のテスト戦略を使い分けることを検討します：

1. **コントラクトテスト**:
   - 目的: モジュールインターフェースの正しさを検証
   - 特徴: 実際のモジュールに依存せず、モックのみでテスト
   - ユースケース: 外部APIや環境への依存が強いテスト
   - 限界: 実装の正確性は検証できない

2. **実装テスト**:
   - 目的: 実際の実装の正しさを検証
   - 特徴: 実際のモジュールをインポートし、依存部分のみモック化
   - ユースケース: ビジネスロジックやコアロジックのテスト
   - 推奨: 基本的にはこちらを優先すべき

### MultiTimeframeDataFetcherテストの問題

MultiTimeframeDataFetcher.test.jsの変換作業では一時的にコントラクトテスト方式に変更しましたが、以下の問題が発生:
...
1. **ハイブリッドアプローチの採用**:
   - 基本的には実装テストを維持
   - 必要に応じてコントラクトテストを補足的に追加（別ファイル）
   - 実装テストが困難な場合のみコントラクトテストに置き換え

2. **モジュールモック戦略**:
   - 完全なモジュールモックは極力避ける
   - 代わりに`jest.spyOn()`で特定メソッドのみモック化
   - 依存関係の分離（dependency isolation）を徹底

3. **変換ステップ**:
   - Step 1: 最小限の変更でCommonJS形式に変換
   - Step 2: 問題のある依存関係をモック化
   - Step 3: テスト機能を保持しながらTypeScript固有機能を置換

実装と乖離したテストは、バグ検出能力が大幅に低下するため、最後の手段としてのみ検討すべきです。
